...
- **Testing a paraphrase instead of the proposal.** When checking someone's proposed method, run it exactly as stated, not a rewrite you believe is equivalent: "equivalent" rewrites smuggle in assumptions (e.g. `avg_over_time(x[w]) * w` equals `sum_over_time(x[w]) * step` only when every series covers the whole window). Run the same math on identical raw data as a control, so method error is separated from pipeline error, and scan many windows before calling it refuted or passing: a handful of instants misses the tail.
