...
- **After it is enabled from the UI, re-request on each new HEAD with the login form.** Once someone has added Copilot from the PR's browser Reviewers menu, `gh pr edit <n> --add-reviewer copilot-pull-request-reviewer` (the bare login, no `[bot]` suffix) re-requests a fresh review on each subsequent push. Use this form, not `@copilot`.
