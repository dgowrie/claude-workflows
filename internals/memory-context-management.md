...
`memory-hygiene.md` is symlinked into `~/.claude/rules/` and loads into every session, so it carries the directive without the reasoning. `session-wrapup` is a skill (moved from a rule in a later context-cleanup pass) and loads on invocation instead, carrying its directive only when triggered. The reasoning lives here either way.
