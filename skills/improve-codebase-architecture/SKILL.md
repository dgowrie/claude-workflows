...
Write a single HTML file to the OS temp directory so nothing lands in the repo. Resolve the temp dir from `$TMPDIR`, falling back to `/tmp` (or `%TEMP%` on Windows), and write to `<tmpdir>/architecture-review-<timestamp>.html` so each run gets a fresh file. Keep `<timestamp>` filesystem-safe (Unix epoch seconds, or `YYYYMMDD-HHMMSS`); the colons in an ISO 8601 timestamp are illegal in Windows filenames. Open it for the user and tell them the absolute path: `xdg-open "<path>"` on Linux, `open "<path>"` on macOS, and on Windows `start "" "<path>"` from cmd.exe or `Start-Process "<path>"` from PowerShell. Quote the path every time. The two Windows forms are not interchangeable: cmd's `start` reads a leading quoted argument as the window title, so it needs the empty `""` first, whereas PowerShell's `start` is an alias for `Start-Process`, whose mandatory positional `-FilePath` would bind that same `""` and fail.
