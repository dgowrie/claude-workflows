---
name: check-npm
description: Use when auditing a JavaScript/TypeScript repo's npm/yarn/pnpm package manager configuration for supply-chain hardening. Trigger on /check-npm (or when the user asks to audit package manager security, lifecycle scripts, git dependencies, ignore-scripts, min-release-age, or similar checks).
---
...
- **pnpm ≥ 11**: read `pnpm-workspace.yaml` (and `pnpm` field in `package.json`). `.npmrc` is **auth/registry only** on pnpm 11 - settings written there for build-script control are silently ignored.
...
- **pnpm 10.x** (legacy): read `.npmrc` AND `pnpm-workspace.yaml`.
...
For **all package managers**, scan `dependencies` / `devDependencies` / `optionalDependencies` / `peerDependencies`. List every entry whose value is **not** on the allow-list, formatted as:
...
- **pnpm ≥ 11**: read `pnpm-workspace.yaml` (and `pnpm` field in `package.json`). `.npmrc` entries for this setting are ignored on pnpm 11.
  - PASS if `minimumReleaseAge:` is set to an integer ≥ `4320`.
  - **SOFT PASS** if `minimumReleaseAge` is unset: pnpm 11 defaults to `1440` (1 day), so the gate is *active but below the 3-day bar*. Report PASS with detail `default applies (pnpm 11 = 1440 min < 4320)` and recommend setting `minimumReleaseAge: 4320` explicitly.
...
- **pnpm 10.x**: read `.npmrc` AND `pnpm-workspace.yaml`.
...
| 4 | Min release age ≥ 3 days | PASS / FAIL | config line + value, or `default applies (pnpm 11 = 1440 min < 4320)` |
...
**pnpm min release age (v11)** → add to `pnpm-workspace.yaml` (value is **minutes**):
...
- Only read files at the workspace root (do NOT walk subdirectories).
